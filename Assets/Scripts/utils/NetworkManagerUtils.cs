...
namespace VidiGraph
{
    ...
    {
        public static void BulkInitNetwork(NetworkStorage storage, NetworkGlobal networkGlobal, MultiLayoutContext context)
        {
            ...
            try
            {
                ...
                var nfc = ConvertToNeoPath(fc, storage.ConvertWinPaths);
                var nfn = ConvertToNeoPath(fn, storage.ConvertWinPaths);
                var nfn2n = ConvertToNeoPath(fn2n, storage.ConvertWinPaths);
        ...
        static void DumpNetwork(NetworkGlobal networkGlobal, MultiLayoutContext context,
            out string commFile, out string nodeFile, out string nodeToNodeFile)
        {
            ...
            using (StreamWriter cFile = new StreamWriter(commFile),
                                    nFile = new StreamWriter(nodeFile),
                                    n2nFile = new StreamWriter(nodeToNodeFile))
            {
