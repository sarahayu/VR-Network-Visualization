...
Shader "Custom/Terrain Surface"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_NodeColTex);
            SAMPLER(sampler_NodeColTex);
            TEXTURE2D(_SelectionTex);
            SAMPLER(sampler_SelectionTex);
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                half4 linkLineCol = SAMPLE_TEXTURE2D(_LineTex, sampler_LineTex, IN.uv);
                half4 nodeCol = SAMPLE_TEXTURE2D(_NodeColTex, sampler_NodeColTex, IN.uv);
                float heightFactor;
                if (_UseHeightMap == 1)
                    heightFactor = SAMPLE_TEXTURE2D(_HeightMap, sampler_HeightMap, IN.uv);
                else
                    heightFactor = inverseLerp(0, _MaxHeight + 0.01, getSphericalHeight(IN.positionOS));
                ...
                half4 nodeColAndBg = half4(_Color.rgb * (1 - nodeCol.a) + nodeCol.rgb * nodeCol.a, 1);

                // use node colors and background as base colors, then screen with contour line texture and link lines
                // color = 1 - (1 - nodeColAndBg) * (1 - (linkLineCol.rgb));
                color.rgb = 1 - (1 - nodeColAndBg) * (1 - (stepFactor + linkLineCol.rgb));

                // add selection highlighting
                half4 selCol = SAMPLE_TEXTURE2D(_SelectionTex, sampler_SelectionTex, IN.uv);
                
                // if transparency set to 0, we are only doing outlines, so just add color (this is my own convention)
                if (selCol.a == 0)
                    color.rgb += selCol.rgb;
                ...
                // o.Normal = UnpackNormal (SAMPLE_TEXTURE2D(_BumpMap, sampler_BumpMap, IN.uv));
                color.a = linkLineCol.a;
